...
export default function ReceiptViewPage() {
  ...
  if (error || !p) return <div className="p-8 text-sm text-red-600">Receipt not found.</div>;

  const party = p.direction === 'RECEIVED' ? p.customer : p.supplier;

  return (
    <ErpPage
      kicker={p.direction === 'RECEIVED' ? 'Receipt Voucher' : 'Payment Voucher'}
      title={p.number}
      description={`${party?.name ?? ''} · AED ${n2(p.amount)}`}
      actions={
        <>
          <Link href="/erp/receipts" className="inline-flex items-center gap-1.5 rounded-sm border border-ink-200 px-3 py-2 text-xs font-bold uppercase tracking-widest text-ink-700 hover:border-brand-500"><ArrowLeft className="h-4 w-4" /> Back</Link>
    ...
    >
      <div className="space-y-5">
        <DocHeader
          party={party}
          partyLabel={p.direction === 'RECEIVED' ? 'Received From' : 'Paid To'}
          meta={[
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between border-b border-ink-100 pb-4">
            <span className="font-display text-lg tracking-widest text-ink-900">AMOUNT {p.direction === 'RECEIVED' ? 'RECEIVED' : 'PAID'}</span>
            <span className="font-mono text-2xl font-bold text-green-700">AED {n2(p.amount)}</span>
          </div>
              ...
              <tbody>
                {p.allocations.map((a) => (
                  <tr key={a.id} className="border-b border-ink-100 last:border-b-0">
                    <td className="py-2 font-mono text-xs">{a.invoice?.number ?? a.purchaseInvoice?.number ?? '—'}</td>
                    <td className="py-2 text-right font-mono font-bold">{n2(a.amount)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
