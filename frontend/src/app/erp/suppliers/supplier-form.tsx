...
import { useCreate, useUpdate, useDelete, type Supplier } from '@/lib/erp-api';
...
interface SupplierInput {
  ...
  creditDays: number; openingBalance: number;
...
const blank: SupplierInput = {
  ...
  creditDays: 0, openingBalance: 0, notes: '', isActive: true,
};

export function SupplierForm({ supplier }: { supplier?: Supplier }) {
  const router = useRouter();
  const can = useAuth((s) => s.can);
  const create = useCreate<SupplierInput>('suppliers');
  const update = useUpdate<SupplierInput>('suppliers');
  const del    = useDelete('suppliers');

  const isEdit = !!supplier;
  const [form, setForm] = useState<SupplierInput>(() =>
    supplier ? {
      code: supplier.code, name: supplier.name, legalName: supplier.legalName ?? '',
      trn: supplier.trn ?? '', email: supplier.email ?? '', phone: supplier.phone ?? '',
      contactPerson: supplier.contactPerson ?? '', addressLine1: supplier.addressLine1 ?? '',
      addressLine2: '', city: supplier.city ?? '', country: 'United Arab Emirates',
      creditDays: supplier.creditDays, openingBalance: Number(supplier.openingBalance),
      notes: '', isActive: supplier.isActive,
    } : blank,
  );
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      if (isEdit && supplier) await update.mutateAsync({ id: supplier.id, body: form });
  ...
  return (
    <ErpPage
      kicker={isEdit ? 'Edit supplier' : 'New supplier'}
      title={isEdit ? (supplier?.name ?? 'Supplier') : 'Add a supplier'}
      description={isEdit ? `Code ${supplier?.code}` : 'Create a new supplier record.'}
      actions={
        <Link href="/erp/suppliers" className="inline-flex items-center gap-1.5 rounded-sm border border-ink-200 px-3 py-2 text-xs font-bold uppercase tracking-widest text-ink-700 hover:border-brand-500 hover:text-brand-600">
    ...
    >
      <form onSubmit={onSubmit} className="grid gap-5 lg:grid-cols-[1fr_320px]">
          ...
          <Card className="p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <Field label="Supplier code" required>
                <input required value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} className={`${inputCls} font-mono`} placeholder="e.g. SUPP-0001" />
              </Field>
        ...
        <div className="space-y-5">
          <Card className="p-6">
            <h2 className="mb-4 font-display text-lg tracking-widest text-ink-900">TERMS</h2>
          ...
          <div className="flex flex-col gap-2">
            <button type="submit" disabled={create.isPending || update.isPending || !can('supplier.write')} className="btn-shimmer inline-flex items-center justify-center gap-2 rounded-sm bg-brand-500 px-5 py-3 text-sm font-bold uppercase tracking-widest text-ink-950 shadow-yellow hover:bg-brand-400 disabled:opacity-60">
              <Save className="h-4 w-4" /> {isEdit ? 'Save changes' : 'Create supplier'}
            </button>
            {isEdit && can('supplier.write') && (
